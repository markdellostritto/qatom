	...
	try{
		...
		while(fgets(input,string::M,reader)!=NULL){
			...
			if(std::strstr(input,"ITEM: ATOMS")!=NULL){
				...
				for(unsigned int i=0; i<format.N; ++i){
...
void read(const char* file, Format& format){
	...
	try{
		...
		while(fgets(input,string::M,reader)!=NULL){
			...
			if(strlist.size()==0) continue;
				...
				if(strlist.at(0)=="Atoms"){
					...
					fgets(input,string::M,reader);
					for(unsigned int i=0; i<format.nSpecies; ++i){
						fgets(input,string::M,reader);
						string::trim_right(input,"#");
						std::strtok(input,string::WS);
		...
		std::rewind(reader);
		std::vector<unsigned int> indices(format.nSpecies,0);
		while(fgets(input,string::M,reader)!=NULL){
			string::trim_right(input,"#");
			string::split(input,string::WS,strlist);
			if(strlist.size()==0) continue;
			if(strlist.at(0)=="Atoms"){
				...
				if(format.styleAtom==STYLE_ATOM::FULL){
					for(unsigned int i=0; i<format.N; ++i){
						//read in the specie
						fgets(input,string::M,reader);
						unsigned int index=std::atoi(std::strtok(input,string::WS));
						std::strtok(NULL,string::WS);
		...
		if(LAMMPS_PRINT_DATA>0){
			...
			std::cout<<"SPECIES   = "; for(unsigned int i=0; i<format.name.size(); ++i) std::cout<<format.name[i]<<" "; std::cout<<"\n";
			std::cout<<"NUMBERS   = "; for(unsigned int i=0; i<format.natoms.size(); ++i) std::cout<<format.natoms[i]<<" "; std::cout<<"\n";
		}
	}catch(std::exception& e){
		std::cout<<"ERROR in "<<NAMESPACE_GLOBAL<<"::"<<NAMESPACE_LOCAL<<"::"<<funcName<<":\n";
		std::cout<<e.what()<<"\n";
	...
	try{
		...
		if(s!=1.0){
			for(unsigned int t=0; t<sim.timesteps(); ++t){
				sim.frame(t).cell().init(sim.frame(t).cell().R()*s);
			}
			for(unsigned int t=0; t<sim.timesteps(); ++t){
				for(unsigned int n=0; n<sim.frame(t).nAtoms(); ++n){
