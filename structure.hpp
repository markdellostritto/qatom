...
public:
	...
	int id(unsigned int specie, unsigned index)const{return offsets_[specie]+index;};
	...
	static std::vector<unsigned int>& read_atoms(const char* str, std::vector<unsigned int>& ids, const StructureI& strucI);
	static unsigned int read_natoms(const char* str);
...
protected:
	...
	ListData<int>				molecule_;	//molecule
public:
	//constructors/destructors
	StructureS(){};
	...
	friend std::ostream& operator<<(std::ostream& out, const StructureS& struc){return out;};
	...
	//molecule
		const ListData<int>& molecule()const{return molecule_;};
...
public:
	...
	//molecule
		int& molecule(unsigned int i){return molecule_[i];};
		const int& molecule(unsigned int i)const{return molecule_[i];};
		int& molecule(unsigned int i, unsigned int j){return molecule_[offsets_[i]+j];};
		const int& molecule(unsigned int i, unsigned int j)const{return molecule_[offsets_[i]+j];};
...
struct Utility{
	struct Read{
		static void chg_atom(const char* file, Simulation& sim);
		static void alpha_atom(const char* file, Simulation& sim);
	};
	struct Write{
		static void chg_tot(const char* file, const Simulation& sim);
		static void chg_atom(const char* file, const Simulation& sim);
		static void dipole_tot(const char* file, const Simulation& sim);
		static void alpha_tot(const char* file, const Simulation& sim);
		static void alpha_atom(const char* file, const Simulation& sim);
	};
};
